...
export default function CreateBukuPage() {
  ...
  useEffect(() => {
    fetch('/api/perpustakaan/buku?limit=1').then(r => r.json()).then(d => {
      setKategoriList(d.filters?.kategoriList || []);
      setRakList(d.filters?.rakList || []);
    });
  }, []);

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.judul) { toast('error', 'Judul buku wajib diisi'); return; }
    setLoading(true);
    const res = await fetch('/api/perpustakaan/buku', {
    ...
    setLoading(false);
    if (res.ok) { toast('success', 'Buku berhasil ditambahkan'); router.push('/admin/perpustakaan/buku'); }
    else toast('error', 'Gagal menambahkan buku');
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-secondary-900">Tambah Buku</h1>
      </div>
      ...
      <Card className="p-6 max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <Input id="stok_minimum" label="Stok Minimum" type="number" value={form.stok_minimum} onChange={set('stok_minimum')} min="0" />
            <Input id="halaman" label="Jumlah Halaman" type="number" value={form.halaman} onChange={set('halaman')} />
          </div>
