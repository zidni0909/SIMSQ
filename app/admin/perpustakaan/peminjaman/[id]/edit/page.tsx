...
export default function EditPeminjamanBukuPage() {
  const router = useRouter();
  const params = useParams();
  const [loading, setLoading] = useState(false);
  const [fetching, setFetching] = useState(true);
  const [data, setData] = useState<any>(null);
  const [form, setForm] = useState({ tanggal_kembali_rencana: '', catatan: '' });

  useEffect(() => {
    fetch(`/api/perpustakaan/peminjaman/${params.id}`).then(r => r.json()).then(d => {
      setData(d);
      setForm({ tanggal_kembali_rencana: d.tanggal_kembali_rencana?.split('T')[0] || '', catatan: d.catatan || '' });
      setFetching(false);
    });
  }, [params.id]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setLoading(true);
    const res = await fetch(`/api/perpustakaan/peminjaman/${params.id}`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form),
    });
    setLoading(false);
    if (res.ok) { toast('success', 'Data berhasil diupdate'); router.push('/admin/perpustakaan/peminjaman'); }
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-secondary-900">Edit Peminjaman Buku</h1>
      </div>

      <Card className="p-6 max-w-2xl">
        <div className="mb-4 p-3 bg-secondary-50 rounded-lg text-sm space-y-1">
          <p><strong>Kode:</strong> {data?.kode_transaksi}</p>
          <p><strong>Buku:</strong> {data?.buku_judul}</p>
          <p><strong>Anggota:</strong> {data?.anggota_nama}</p>
          <p><strong>Status:</strong> {data?.status}</p>
          {data?.denda > 0 && <p><strong>Denda:</strong> {formatCurrency(data.denda)}</p>}
        </div>
