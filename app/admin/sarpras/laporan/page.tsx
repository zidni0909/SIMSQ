...
export default function LaporanSarprasPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-secondary-900 mb-6">Laporan Sarpras</h1>
          ...
          {Object.entries(data.summary).map(([key, value]) => (
            <Card key={key} className="p-4">
              ...
              <p className="text-xl font-bold text-secondary-900">
                {key.includes('nilai') || key.includes('biaya') ? formatCurrency(Number(value)) : String(value ?? 0)}
              </p>
            </Card>
          ))}
        </div>
                  ...
                  <tr key={idx} className="border-b border-secondary-100 hover:bg-secondary-50">
                    {Object.entries(row).map(([key, val]) => (
                      <td key={key} className="py-2 px-3 text-xs whitespace-nowrap">
                        {key === 'status' || key === 'kondisi' ? <StatusBadge status={String(val)} /> : key.includes('harga') || key.includes('biaya') ? formatCurrency(Number(val)) : String(val ?? '-')}
                      </td>
                    ))}
                  </tr>
                ))}</tbody>
