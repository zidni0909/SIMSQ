...
export default function LabelBarangPage() {
  const [items, setItems] = useState<BarangLabel[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    const res = await fetch(`/api/sarpras/barang/label?${params}`);
  ...
  async function generatePDF() {
    ...
    setGenerating(true);
    try {
      const res = await fetch(`/api/sarpras/barang/label?ids=${selected.join(',')}`);
      const json = await res.json();
      const selectedItems: BarangLabel[] = json.data;
      ...
      for (let i = 0; i < selectedItems.length; i++) {
        ...
        doc.text(item.kode_barang, textX, y + 7);

        // Nama barang (wrap if needed)
        doc.setFontSize(6.5);
        doc.setFont('helvetica', 'normal');
        const namaLines = doc.splitTextToSize(item.nama, textMaxW);
        doc.text(namaLines.slice(0, 2), textX, y + 12);

        // Lokasi
        if (item.lokasi_nama) {
          doc.setFontSize(5.5);
          doc.setTextColor(100);
          doc.text(item.lokasi_nama, textX, y + 21);
          doc.setTextColor(0);
        }
        ...
        if (item.kategori_nama) {
          ...
          doc.text(item.kategori_nama, textX, y + 25);
          doc.setTextColor(0);
        }

        // QR Code (right side)
        try {
          const qrUrl = `${baseUrl}/admin/sarpras/barang/${item.id}`;
      ...
      doc.save('label-qr-barang-sarpras.pdf');
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-secondary-900">Label QR Code Barang</h1>
        ...
        {selected.length > 0 && (
          <div className="mb-4 p-3 bg-primary-50 rounded-lg flex items-center justify-between">
            <span className="text-sm text-primary-700 font-medium">
              <Tag size={14} className="inline mr-1" /> {selected.length} barang dipilih
            </span>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-secondary-200">
                  ...
                  <th className="text-left py-3 px-4 font-medium text-secondary-500">Merk</th>
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id} className={`border-b border-secondary-100 hover:bg-secondary-50 cursor-pointer ${selected.includes(item.id) ? 'bg-primary-50' : ''}`} onClick={() => toggleSelect(item.id)}>
                    ...
                    <td className="py-3 px-4 text-secondary-600">{item.merk || '-'}</td>
