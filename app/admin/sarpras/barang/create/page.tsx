...
export default function CreateBarangPage() {
  ...
  const [form, setForm] = useState({
    ...
    sumber_dana: '', harga_perolehan: '', jumlah: '1', kondisi: 'baik', keterangan: '',
    penanggung_jawab: '', jabatan_pj: '', kontak_pj: '',
  });

  useEffect(() => {
    fetch('/api/sarpras/barang?limit=1').then(r => r.json()).then(d => {
      setKategoriList(d.filters?.kategoriList || []);
      setLokasiList(d.filters?.lokasiList || []);
    });
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.nama) { toast('error', 'Nama barang wajib diisi'); return; }
    setLoading(true);
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/admin/sarpras/barang">
          <button className="p-2 rounded-lg hover:bg-secondary-100"><ArrowLeft size={20} /></button>
        </Link>
        <h1 className="text-2xl font-bold text-secondary-900">Tambah Barang</h1>
      </div>
            ...
            <div className="flex items-center gap-4">
              {fotoPreview ? (
                <div className="relative w-24 h-24">
                  ...
                  <button type="button" onClick={() => { setFotoPreview(null); setFotoFile(null); }} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-0.5">
                    <X size={14} />
                  </button>
                </div>
              ) : (
